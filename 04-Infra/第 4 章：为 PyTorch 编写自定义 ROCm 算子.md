...
在第 3 章中，我们脱掉 Python 的外衣，用 C++ 和 HIP 语言写出了极致性能的底层算子。但在实际的 AI 开发中99% 的时间都在使用 Python。如果我们只能写出独立的 C++ 脚本，那毫无用武之地。
...
![image-20260430164017066](https://raw.githubusercontent.com/wlkq151172/image_store/main/20260430164017122.png)
...
## 🛠️ 4.2 进阶实战：手搓一个 "Fused Swish" 算子
...
// ⚠️ 4. 显式实例化模板（告诉编译器我们需要编译哪些数据类型的版本）
// 这里我们实例化了 float 类型。
...
// 如果你的模型需要支持半精度 (FP16)，可以取消下面的注释：
// #include <hip/hip_fp16.h>
// template void launch_fused_swish_forward<__half>(const __half*, __half*, int);
// template void launch_fused_swish_backward<__half>(const __half*, const __half*, __half*, int);
```
...
这一层是 C++ 世界通往 Python 世界的大门。我们要在这里实现严格的安全检查，以及动态类型分发（Dynamic Dispatch）。因为 Python 是动态类型的，传过来的 Tensor 可能是 float32，也可能是 float16，我们需要宏 `AT_DISPATCH_FLOATING_TYPES` 帮我们在运行时绑定到对应的 C++ 模板上。
...
torch::Tensor fused_swish_forward(torch::Tensor input) {
    ...
    // "fused_swish_forward" 是出错时显示的提示信息
...
...
hipcc -DNDEBUG -O3 ... -c fused_swish_wrapper.cpp -o build/temp.linux-x86_64-cpython-310/fused_swish_wrapper.o -O3
hipcc -DNDEBUG -O3 ... -c fused_swish_kernel.hip -o build/temp.linux-x86_64-cpython-310/fused_swish_kernel.o -O3
g++ -pthread -shared ... -o build/lib.linux-x86_64-cpython-310/my_custom_swish_backend.cpython-310-x86_64-linux-gnu.so
...
running install_lib
copying build/lib.linux-x86_64-cpython-310/my_custom_swish_backend.cpython-310-x86_64-linux-gnu.so -> /your/python/environment/lib/python3.10/site-packages
running install_egg_info
Writing /your/python/environment/lib/python3.10/site-packages/my_custom_swish-0.0.0-py3.10.egg-info
...
我们在 `test_swish.py` 的末尾追加一段**的 Benchmark**：
