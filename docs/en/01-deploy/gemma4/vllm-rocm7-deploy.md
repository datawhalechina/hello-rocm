...
This section explains how to use **vLLM** for Gemma 4 inference on Ubuntu 24.04 + ROCm 7+, including:
...
The example model is **Gemma 4 E4B-it (`google/gemma-4-E4B-it`)**; if you have sufficient VRAM, you can also use `google/gemma-4-31B-it` or `google/gemma-4-26B-A4B-it`.
...
The container's `/app/models` is mounted to the host's `~/models` (used for caching Hugging Face downloaded model weights).

> Gemma series requires accepting the model terms of use on Hugging Face. Before starting, it is recommended to generate a token with read permissions on HF and inject it into the container via `HF_TOKEN`.

---

### 2. Start the Model Service Inside the Container

Inside the container, start Gemma 4 E4B-it directly using native HF weights:

```bash
# Standard start (with CUDA/HIP graph enabled, faster inference)
vllm serve google/gemma-4-E4B-it \
  --dtype bfloat16 \
  --max-model-len 8192 \
  --max-num-seqs 32

# Quick start (--enforce-eager): disables CUDA/HIP graph,
# faster startup but slightly slower inference (typically 10–20% slower).
vllm serve google/gemma-4-E4B-it \
  --dtype bfloat16 \
  --enforce-eager \
  --max-num-seqs 32 \
...
> Notes:
> - Gemma 4 E4B natively supports **128K** context. The example only sets `--max-model-len 8192` to run smoothly with limited VRAM; you can gradually increase it based on available VRAM.
> - To enable vision / audio multimodal input, refer to the latest vLLM multimodal documentation for Gemma 4 and the corresponding `--limit-mm-per-prompt`, `--served-model-name`, and other parameters.

---

### 3. Performance Testing Script (tokens/s)

Below is a complete Bash script example for measuring the actual inference speed of **Gemma 4 E4B-it**:
...
Test result example (Gemma 4 E4B-it, ctx=4096):
...
# After logging in to HF (huggingface-cli login) and accepting Gemma 4 terms of use,
# you can start the service directly using the repository ID
vllm serve google/gemma-4-E4B-it \
  --dtype bfloat16 \
  --max-model-len 8192 \
  --max-num-seqs 32
