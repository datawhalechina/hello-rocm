...
<div align='center'>
    <img src="/images/03-infra/decode-ai-accelerator/python_to_gpu_overview_flow.png" alt="图2.1 从 Python 到 GPU 的总览流程" width="95%">
...
<div align='center'>
    <img src="/images/03-infra/decode-ai-accelerator/ldd_pytorch_dependency_chain.png" alt="图2.2 ldd 追踪 PyTorch 依赖链路" width="90%">
...
<div align='center'>
    <img src="/images/03-infra/decode-ai-accelerator/hip_llvm_isa_compile_pipeline.png" alt="图2.3 HIP / LLVM / ISA 编译链路图" width="95%">
...
<div align='center'>
    <img src="/images/03-infra/decode-ai-accelerator/hip_cpu_gpu_swimlane.png" alt="图2.4 HIP 程序 CPU-GPU 双泳道执行图" width="95%">
...
<div align='center'>
    <img src="/images/03-infra/decode-ai-accelerator/cpu_vs_gpu_philosophy.png" alt="图2.5 CPU vs GPU 低延迟与高吞吐对比" width="90%">
    <p><b>图2.5</b> CPU 低延迟 vs GPU 高吞吐设计哲学对比</p>
</div>

<!-- <div align='center'>
    <img src="/images/03-infra/decode-ai-accelerator/cpu_latency_gpu_throughput.png" alt="图2.6 CPU 低延迟 vs GPU 高吞吐" width="95%">
...
<div align='center'>
    <img src="/images/03-infra/decode-ai-accelerator/resnet50_benchmark_result.png" alt="图2.7 ResNet-50 CPU vs GPU 性能对比" width="90%">
...
<div align='center'>
    <img src="/images/03-infra/decode-ai-accelerator/simt_wavefront_execution.png" alt="图2.8 SIMT / Wavefront 执行模型图" width="95%">
    <p><b>图2.8</b> SIMT / Wavefront 执行模型：同一条指令，多个线程处理不同数据</p>
</div>

<div align='center'>
    <img src="/images/03-infra/decode-ai-accelerator/simt_control_vs_alu_ratio.png" alt="图2.9 CPU 控制单元 vs GPU 运算单元比例差异" width="90%">
...
<div align='center'>
    <img src="/images/03-infra/decode-ai-accelerator/wavefront_divergence.png" alt="图2.10 分支发散 Divergence 示意图" width="95%">
...
<div align='center'>
    <img src="/images/03-infra/decode-ai-accelerator/coalesced_vs_strided_access.png" alt="图2.11 合并访问 vs 随机访问" width="95%">
...
<div align='center'>
    <img src="/images/03-infra/decode-ai-accelerator/amd_gpu_cu_internal_structure.png" alt="图2.12 AMD GPU CU 内部结构图" width="95%">
...
graph TB
    subgraph CU["Compute Unit (CU)"]
        direction TB
        SIMD["SIMD Units<br/>4 个 SIMD × 64 ALU = 256 个标量 ALU"]
        REG["Register File (VGPR)<br/>32 字节寄存器，存储线程私有数据"]
        LDS["LDS Local Data Store<br/>128 KB，比显存快 10 倍"]
        CTRL["控制单元<br/>Instruction Cache, Scalar Unit"]
    end
...
<div align='center'>
    <img src="/images/03-infra/decode-ai-accelerator/occupancy_register_pressure.png" alt="图2.13 Occupancy 与寄存器压力图" width="95%">
...
本章涉及的完整源码位于 `src/infra/decode-ai-accelerator/code/` 目录：

| 文件 | 说明 |
|:---|:---|
| `hello_rocm.cpp` | 第一个 HIP 程序：向量加法验证 |
| `simple_add.cpp` | HIP 编译链路演示：LLVM IR / ISA 输出 |
| `bench_resnet.py` | ResNet-50 CPU vs GPU 推理性能对比 |
| `bench_divergence.cpp` | 分支发散性能影响测试（reduction 对比） |
| `bench_bandwidth.py` | PCIe vs GPU 显存带宽测试 |
| `calc_occupancy.py` | 理论 Occupancy 计算脚本 |
...
- 用 `ldd` 追踪了 PyTorch → HIP → HSA → 驱动 → GPU 的完整调用链路。
- 理解了 CPU「低延迟」与 GPU「高吞吐」的设计哲学差异，以及 SIMT 执行模型。
- 深入 AMD GPU 硬件架构，掌握了 CU、VGPR、LDS、显存带宽等核心概念。
- 通过 rocprof 实测了 Occupancy、分支发散、内存带宽等关键性能指标。
