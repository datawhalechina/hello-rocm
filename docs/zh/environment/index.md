...
&emsp;&emsp;后续所有章节（01-Deploy、02-Fine-tune 等）的环境准备均以本章为基准。如需使用其他 ROCm 版本或其他 GPU 架构，请参考 [GPU 架构对照表](/zh/environment/rocm-gpu-architecture-table) 进行对应替换。
...
- [GPU 架构对照表（独立文件）](/zh/environment/rocm-gpu-architecture-table)
...
>
> 完整对照请查阅 [GPU 架构对照表](/zh/environment/rocm-gpu-architecture-table) 或 [官方兼容性矩阵](https://rocm.docs.amd.com/en/7.12.0-preview/compatibility/compatibility-matrix.html)。
...
> 💡 其他 GPU 替换 `--index-url` 即可，对照表见 [Windows 1.5 节](#15-安装-rocm--pytorch) 或 [GPU 架构对照表](/zh/environment/rocm-gpu-architecture-table)。
...
完整对照表见 [GPU 架构对照表](/zh/environment/rocm-gpu-architecture-table)。
