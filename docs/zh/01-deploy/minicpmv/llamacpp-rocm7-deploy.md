## llama.cpp 部署 MiniCPM-V（Ubuntu 24.04 + ROCm 10+）
...
参考性能（Radeon 8060S，gfx1151，原实测 ROCm 7.13，ctx=4096）：文本和多模态解码均约 **190 tokens/s**（首轮多模态请求另含图像编码时间）。环境命令已对齐 10.0.0，实际速度取决于硬件。
