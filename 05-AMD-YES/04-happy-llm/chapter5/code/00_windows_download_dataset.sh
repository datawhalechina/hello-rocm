...
# 使用PowerShell下载
# Set env vars for current PowerShell session only (cleared when shell exits)
# 暂时为当前PowerShell界面设置环境，关闭Powershell环境自动消失
$env:HF_ENDPOINT = "https://hf-mirror.com"

# Replace \path\to\your\dataset with your target dataset directory
# 将\path\to\your\dataset替换成想要下载dataset目录地址
$dataset_dir = "\path\to\your\dataset"

# Install modelscope in advance: pip install modelscope
...
# Download with CMD
# 使用CMD下载
# Set env vars for current CMD session only (cleared when shell exits)
